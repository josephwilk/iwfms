:-ensure_loaded(library(objects)).
:-ensure_loaded('lnowledgeBase/library_db.pl').
:- use_module(library(pillow)).

main :- 
  icon_address(dot,Dot),
  % [MC] tell('html_demo.html'),
  output_html([
    start,
    ...
    --,
    ref('mailto:mwahhhhhhhhhhhhhhclip@dia.fi.upm.es',address('clip@dia.fi.upm.es')),
    end
  ]),
  % [MC] told,
  true.
