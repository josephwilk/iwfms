...
axiom( happens(formCreation(admissionForm,User), T1,T12),[
	
	notOccured(happens(formEntry(admissionForm,User,receptionist),Tx,Tx)),

	valid_html_form( input, [size=30] ),
	happens( createFormElement( textbox,"Patient Name", patientName,'', [size=30]),T1),
		
	valid_html_form( input, [size=50] ),
	happens( createFormElement(textbox,"NurseID Number", nurseID, '',[size=50] ),T2),
	before(T1,T2),
	...
	happens( createFormElement(textbox,"House Number", houseNumber, '',[size=50] ),T3),
	before(T2,T3),
	
	valid_html_form( input, [disabled,size=30] ),
	happens( createFormElement(textbox,"Road", road, '',[disabled,size=50] ),T4),
	before(T3,T4),
	
	valid_html_form( input, [disabled,size=30] ),
	happens( createFormElement(textbox,"Age", age,'', [disabled,size=50] ),T5),
	before(T4,T5),
	
	valid_html_form( select, [size=1] ),
	valid_html_formChildren( select, [option] ),
	happens( createFormElement(select,"Ward", ward, '', [size=1] ),T6),
	before(T5,T6),

	
	happens( createFormElement(option,"Please select a Ward", empty, '', [] ),T7),
	before(T6,T7),
	
	happens( createFormElement(option,"Adults Ward", childrensWard, 'adultWard', [] ),T8),
	before(T7,T8),
	
	happens( createFormElement(option,"Childrens Ward", adultWard, 'childrensWard', [] ),T9),
	before(T8,T9),
		
	happens( createFormElement(endSelect,'', ward, '', [] ),T10),
	before(T9,T10),
	
	happens(formInput(admissionForm),T11, T12),
	before(T10,T11)	
...
axiom( happens(formCreation(admissionForm,User), T1,T11),[
	...
	notOccured(happens(formEntry(admissionForm,User,staffNurse),Tx,Tx)),
...
axiom( happens(formCreation(assessForm,User), T1,T1),[
	...
	happens( createFormElement(textarea, "Drugs perscription", drugs, '', [rows=30,cols=30]), T1)
