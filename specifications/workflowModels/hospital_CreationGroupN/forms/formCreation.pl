...
axiom( happens(formCreation(superForm,User), T1,T2),[
	
	valid_html_form( input, [type=radio] ),
	happens( createFormElement(textbox,"Super User ID", superID, '', []),T1),
	
	valid_html_form( input, [type=radio] ),
	happens( createFormElement(textbox,"Creation reason", reason, '', []),T2),
...
axiom( happens(formCreation(admissionForm,User), T1,T11),[
	...
	valid_html_form( input, [type=text] ),
