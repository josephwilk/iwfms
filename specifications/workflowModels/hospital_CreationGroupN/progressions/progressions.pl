...
axiom(initiates( edgeProgression(superForm, formElement(    superID, relativeExpression, 'number') ), 
		  validated(form(superForm, formElement(    superID, relativeExpression, 'number' ) ) ), T),[
]).
...
axiom(initiates( edgeProgression(pharmacyForm, formElement( pharmacyCollection ,match, true ) ), 
		  validated(form(pharmacyForm, formElement( pharmacyCollection ,match, true) ) ), T),[
]).
